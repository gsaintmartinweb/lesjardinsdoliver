  ...
  return (
    <>
      <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
        <Dialog.Trigger className="flex flex-col align-middle ">
          <button className="bg-gray-200 text-black p-2 rounded w-1/5 hover:bg-gray-300 hover:font-bold flex items-center space-x-2">
            <PlusCircledIcon className="mr-4" /> <span>Ajouter une plante</span>
          </button>
        </Dialog.Trigger>
        <Dialog.Overlay className="fixed inset-0 bg-black opacity-30 z-30" />
        <Dialog.Content className="fixed left-1/2 top-1/2 max-h-[85vh] w-[90vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-gray1 p-[25px] shadow-[var(--shadow-6)] focus:outline-none data-[state=open]:animate-contentShow overflow-y-auto z-50">
